...
namespace rev {
	...
	bool Player::init(Window _window, const std::string& scene, const std::string& bg) {
		...
		if(mGfxDriver) {
			...
			// Default scene light
			{
				...
				lightXform.setRotation(Quatf(Vec3f(1.f, 0.6f, 0.2f).normalized(), -Constants<float>::halfPi));
	...
	void Player::createCamera() {
		if(mGraphicsScene.cameras().empty())
		{
			...
			cameraNode->addComponent<Transform>()->xForm.position() = math::Vec3f { 0.f, 0.f, 0.f };
