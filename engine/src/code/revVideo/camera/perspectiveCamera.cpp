...
namespace rev { namespace video
{
	...
	{
		// Precomputations
		TReal xFactor = 1.f / cos(180.f * _fov / 3.14159f); // One over cosine of the field of view
		TReal yFactor = xFactor * _invRatio;
		TReal invZRange = 1.f / (_clipDistances.y - _clipDistances.x);
		// Row 0
		mProjection.m[0][0] = xFactor;
		...
		mProjection.m[1][2] = yFactor; // Map height from Z to Y
		mProjection.m[1][3] = 0.f;
		// Row 2   
		mProjection.m[2][0] = 0.f;
		mProjection.m[2][1] = 2.f * invZRange;
		mProjection.m[2][2] = 0.f;
		mProjection.m[2][3] = - (_clipDistances.x + _clipDistances.y) * invZRange;
