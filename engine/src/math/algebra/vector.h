...
namespace rev {
	namespace math {
		...
		public:
			...
			Vector() = default;
			Vector(const Vector&) = default;
			Vector(T_ _x, T_ _y, T_ _z, T_ _w = T_(0)) : x(_x), y(_y), z(_z), w(_w) {}
			Vector(const Vector<T_,2>& _v, T_ _z, T_ _w = T_(0)) : x(_v.x), y(_v.y), z(_z), w(_w) {}
			Vector(const Vector<T_,3>& _v, T_ _w = T_(0)) : x(_v.x), y(_v.y), z(_v.z), w(_w) {}
			explicit Vector(const T_ _v) : x(_v), y(_v), z(_v), w(_v) {}
			...
			Vector	operator/	(const T_) const;		// Division by scalar
			Vector&	operator/=	(const T_);				// Division by scalar
			T_		operator*	(const Vector&_v) const {	// Dot product
		...
		// External operators, per component
		template<class T_>
