...
using idDict = std::unordered_map<std::string,std::string>;
...
public:
	...
	void					SetTextureMatrix( const rev::math::Vec3f mat[2] ) { texMat[0] = mat[0]; texMat[1] = mat[1]; }
	void					GetTextureMatrix( rev::math::Vec3f &mat1, rev::math::Vec3f &mat2 ) { mat1 = texMat[0]; mat2 = texMat[1]; }
	void					GetTextureVectors( rev::math::Vec4f v[2] ) const;

protected:
	std::string				material;
	idPlane					plane;
	rev::math::Vec3f		texMat[2];
	rev::math::Vec3f		origin;
};
...
public:
	...
	static idMapBrush *		Parse( idLexer &src, const rev::math::Vec3f &origin, bool newFormat = true, float version = CURRENT_MAP_VERSION );
	static idMapBrush *		ParseQ3( idLexer &src, const rev::math::Vec3f &origin );
	bool					Write( idFile *fp, int primitiveNum, const rev::math::Vec3f &origin ) const;
...
public:
							...
							~idMapPatch() { }
	static idMapPatch *		Parse( idLexer &src, const rev::math::Vec3f &origin, bool patchDef3 = true, float version = CURRENT_MAP_VERSION );
	bool					Write( idFile *fp, int primitiveNum, const rev::math::Vec3f &origin ) const;
	const char *			GetMaterial() const { return material.c_str(); }
	void					SetMaterial( const char *p ) { material = p; }
...
idMapPatch::idMapPatch( int maxPatchWidth, int maxPatchHeight ) {
	type = TYPE_PATCH;
	horzSubdivisions = vertSubdivisions = 0;
	explicitSubdivisions = false;
	width = height = 0;
